...
//
public abstract class ProdutoDAO extends ProdutoDIC {
    //
    ...
    public long incluir(Map<String,String> elemento) throws SQLException{
        ...
        long retorno = db.insert( ProdutoDIC.TABELA_NOME, null, cv);
        db.close();
    ...
    public int alterar(Map<String,String> elemento, String rowid) throws SQLException{
        ...
        this.db = dbHelper.getWritableDatabase();
        int retorno = db.update(ProdutoDIC.TABELA_NOME, cv, ProdutoDIC.ID + "=?", new String[]{rowid.toString()});
        db.close();
    ...
    public int excluir(String rowid)throws SQLException{
        this.db = dbHelper.getWritableDatabase();
        int retorno = db.delete( ProdutoDIC.TABELA_NOME, ProdutoDIC.ID + "=?", new String[]{rowid.toString()});
        db.close();
    ...
    public Cursor listaTodos(String query) {
        ...
        if (query.isEmpty()) {
            return db.query(ProdutoDIC.TABELA_NOME,
            new String[] {ProdutoDIC.ID,
			ProdutoDIC.CODIGO,
			ProdutoDIC.NOME,
			ProdutoDIC.MODELO,
			ProdutoDIC.PRECO},
