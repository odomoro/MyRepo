...
# @Odone
#
# programa que gera as classes e arquivos suporte para o dashboard
#
# pega e testa os argumentos
#

#TODO: fazer copia dos arquivos da estrutura do dashboard para o diretorio da aplicação.


class GeraAppDashBoard
  ...
  def executa(nome_app)
    ...
    java.puts("public class #{nome_classe} extends DashboardActivity{")
    java.puts('')
    java.puts('    @override')
    java.puts('    protected void onCreate(Bundle savedInstanceState){')
    java.puts('        super.onCreate(savedInstanceState);')
    java.puts("        setContentView(R.layout.#{nome_app});")
    java.puts('    }')
    java.puts('')
    java.puts('    @override')
    java.puts('    public void goHome(Context context){')
    java.puts("        final Intent intent = new Intent(context, #{nome_classe}.class);")
    java.puts('        intent.setFlags (Intent.FLAG_ACTIVITY_CLEAR_TOP);')
    java.puts('        context.startActivity (intent);')
    java.puts('    }')
    java.puts('')
    java.puts('    @override')
    java.puts('    public void onClickFeature (View v){')
    java.puts('        int id = v.getId ();')
    java.puts('        switch (id) {')
    ...
    arquivos.each do |arquivo_base_name|
      ...
      nome_classe = nome_arquivo.split('.')[0]
      #
      java.puts('        case R.id.home_btn_feature1 :')
      java.puts("            startActivity (new Intent(getApplicationContext(), #{nome_classe}.class));")
      java.puts('            break;')
    end
    #
    java.puts('        default:')
